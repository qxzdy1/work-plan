...
1. **查看当天日程**：点击左侧日历任意日期，右侧块 B 会同步显示当天日程。
2. **新增任务**：点击右侧日程表右上角 **+** 按钮，填写弹窗后确定。
3. **编辑任务**：点击右侧日程表里的任务块。
4. **删除任务**：点击日程表右上角 🗑️ 进入删除模式，再点击任务块右上角的小叉。
5. **留言**：在左下角块 C 填写昵称和留言，点击提交。
6. **发布公告**：直接在 Supabase Table Editor 里向 `announcements` 表插入一行 `content`，前端会自动读取最新一条并全块展示。
